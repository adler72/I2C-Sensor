...
class ADCPi:
    ...
    def read_voltage(self, channel):
        ...
        if (self.__signbit):
            ...
            voltage = float((raw * (self.__lsb / self.__pga)) * 10.1)
            return float(voltage)

    def read_grad(self, channel):
        # returns the Grad C from the selected adc channel - channels 1 to
        # 8
        raw = self.read_raw(channel)
        if (self.__signbit):
            return float(0.0)  # returned a negative voltage so return 0
        else:
            grad = float((((raw * (self.__lsb / self.__pga)) * 10.1)/10)*200)
            return float(grad)
